...
def fetch_text(query):
    wiki_wiki = wikipediaapi.Wikipedia(
        user_agent='WikiSummary (vadim@vrutkovs.eu)',
        language='en',
        extract_format=wikipediaapi.ExtractFormat.WIKI)
    page = wiki_wiki.page(query)
    if not page.exists():
        raise Exception(f"No page {query} found")
    st.write(f"Summarizing {page.fullurl} ...")
    return page.text


st.title("🔎 TL;DR Wikipedia")
query = st.text_input("Input query to summarize wikipedia article", "")
if query:
